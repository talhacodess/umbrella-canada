  ...
  return (
    <div className="relative">
      ...
      <div className="md:hidden flex justify-between items-center absolute inset-y-0 left-0 right-0 pointer-events-none px-2">
        <button
          className="arrow arrow-left cursor-pointer rounded-full flex justify-center items-center bg-white/95 backdrop-blur-sm border border-gray-300 shadow-lg w-10 h-10 text-[#213554] transition-all duration-300 group z-10 pointer-events-auto active:scale-95"
          onClick={scrollLeft}
          aria-label="Scroll left"
        >
          <LiaAngleLeftSolid size={20} className="text-[#213554] transition-colors duration-300" />
        </button>
        <button
          className="arrow arrow-right cursor-pointer rounded-full flex justify-center items-center bg-white/95 backdrop-blur-sm border border-gray-300 shadow-lg w-10 h-10 text-[#213554] transition-all duration-300 group z-10 pointer-events-auto active:scale-95"
          onClick={scrollRight}
          aria-label="Scroll right"
        >
          <LiaAngleRightSolid size={20} className="text-[#213554] transition-colors duration-300" />
        </button>
      </div>
    </div>
  );
